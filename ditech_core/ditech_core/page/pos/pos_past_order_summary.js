ditech.POS.PastOrderSummary = class {
    ...
    return `<div class="left-section">
            ...
            doc?.table_label
              ...
              : ""
					...
					<div class="cashier">${__("Sold by")}: ${doc.modified_by}</div>
				</div>
				<div class="right-section">
					<div class="paid-amount">${format_currency(
            doc.paid_amount,
            doc.currency,
            flt(doc.paid_amount, 2) % 1 != 0 ? 2 : 0
          )}</div>
  ...
  get_discount_html(doc) {
    if (doc.discount_amount) {
      return `<div class="summary-row-wrapper">
              ...
              doc.discount_amount,
              doc.currency,
              flt(doc.discount_amount, 2) % 1 != 0 ? 2 : 0
            )}</div>
					</div>`;
    } else {
      return ``;
    ...
    let taxes_html = doc.taxes
      .map((t) => {
        ...
        return `
            ...
            doc.currency,
            flt(t.tax_amount_after_discount_amount, 2) % 1 != 0 ? 2 : 0
          )}</div>
				</div>
			`;
      })
  ...
  get_grand_total_html(doc) {
    ...
    if (second_currency && second_currency != currency)
      second_total = ` | <span>${format_currency(
        this.exchange_rate * doc.grand_total,
        second_currency,
        flt(this.exchange_rate * doc.grand_total, 2) % 1 != 0 ? 2 : 0
      )}</span>`;
  ...
  bind_events() {
    ...
    this.$summary_container.on("click", ".cancel-btn", () => {
      this.events.cencel_order(this.doc.name);
    });
  ...
  get_condition_btn_map(after_submission) {
    if (after_submission)
      return [
        {
          condition: true,
          visible_btns: ["Print Receipt", "New Order"],
        },
      ];

    return [
      {
        condition: this.doc.docstatus === 0 && !this.doc.custom_is_invoice,
        visible_btns: ["Edit Order", "Cancel Order"],
      },
      {
  ...
  async attach_document_info(doc) {
    if (this.doc.custom_pos_table)
      doc.table_label = await frappe.db
        .get_value("POS Table", this.doc.custom_pos_table, "label")
