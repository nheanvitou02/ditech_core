ditech.POS.ItemCart = class {
  constructor({ wrapper, events, settings, is_service, is_retail }) {
    ...
    this.guest_count = settings.custom_guest_count;
    this.last_item_code = "";
    this.init_component();
  ...
  toggle_guest_count() {
    this.events.set_guest_number();
  }

  reset_customer_selector() {
    const frm = this.events.get_frm();
    frm && frm.set_value("customer", "");
    this.make_customer_selector();
  ...
  init_cart_components() {
    this.$component.append(
      `<div class="cart-container">
				<div class="abs-cart-container">
					<div class="cart-label"><div class="label-section">${__(
            "Item Cart"
          )} <span class="table-label"></span></div><div class="invoice-section"></div></div>
    ...
    this.$table_label = this.$component.find(".table-label");
  ...
  make_cart_totals_section() {
    ...
    this.$totals_section.append(
      ...
      `<div class="item-qty-total-container">
        ...
        (this.is_service || this.is_retail) &&
        `grid-template-columns: repeat(1, minmax(0, 1fr));`
      }">
				${
          !this.is_retail
            ? `<div class="save-btn ellipsis">${__("Save as Draft")}</div>`
            : ""
        }
				${!this.is_service ? `<div class="checkout-btn">${__("Checkout")}</div>` : ""}
			</div>
  ...
  make_cart_numpad() {
    ...
    this.$numpad_section.append(
      `<div class="numpad-actions" style="${
        (this.is_service || this.is_retail) &&
        `grid-template-columns: repeat(1, minmax(0, 1fr));`
      }">
      ${
        !this.is_retail
          ? `<div class="save-btn" data-button-value="save">${__(
              "Save as Draft"
            )}</div>`
          : ""
      }
  ...
  bind_events() {
    const me = this;
    this.$table_label.on("click", "#table-label", function () {
      me.events.table_selector();
    });

    this.$customer_section.on("click", ".guest-count", function () {
      me.toggle_guest_count();
    });
    ...
    this.$customer_section.on(
      "click",
      ".customer-image, .customer-name-desc",
      function (e) {
    ...
    this.$cart_items_wrapper.on("click", ".cart-item-wrapper", function () {
      ...
      if (frm.doc.custom_is_invoice && me.is_service) return;
      me.events.cart_item_clicked({ name: item_row_name });
      this.numpad_value = "";
      if (me.is_service) {
        setTimeout(() => {
          me.events.numpad_event(this.numpad_value, "qty");
          me.toggle_numpad_field_edit("qty");
        }, 300);
      }
    });
    ...
    this.$component.on("click", ".save-btn", async function () {
      if ($(this).attr("style").indexOf("--text-muted") == -1) return;
  ...
  attach_shortcuts() {
    ...
    this.$component.find(".save-btn").attr("title", `${ctrl_label}+S`);
    frappe.ui.keys.add_shortcut({
      shortcut: "ctrl+s",
      action: () => this.$component.find(".save-btn").click(),
      condition: () =>
        this.$component.is(":visible") &&
        !this.$totals_section.find(".edit-cart-btn").is(":visible"),
      description: __("Save Order"),
      ignore_inputs: true,
      page: cur_page.page.page,
    });
      ...
      df: {
        ...
        onchange: function () {
          ...
          (frm.doc.items || []).forEach((row) => {
            if (this.value > row.discount_percentage || row?.is_edit) {
              if (row?.org_dis == undefined)
                row.org_dis = row.discount_percentage;
              frappe.model.set_value(
                row.doctype,
                row.name,
                "discount_percentage",
                this.value < row.org_dis ? row.org_dis : this.value
              );
              row.is_edit = true;
            }
          });
          frm.refresh_field("items");
          setTimeout(() => me.hide_discount_control(this.value), 500);
        },
      },
  ...
  update_customer_section() {
    const me = this;
    let guest_number = me.events.get_frm()?.doc?.custom_guest_number || 0;
    const {
      customer,
      email_id = "",
      mobile_no = "",
    ...
    if (customer) {
      this.$customer_section.html(
        `<div class="customer-details">
            ...
            <div class="guest-count">${guest_number}</div>
  ...
  async update_totals_section(frm) {
    ...
    var items = [];
    await frm.doc.items.forEach((item) => {
    ...
    let data = {
      ...
      change_amount: frm.doc.change_amount,
      currency: frm.doc.currency,
      second_currency: frm.doc.custom_second_currency,
      exchange_rate: this.exchange_rate,
      taxes: frm.doc.taxes,
      image: items.find((i) => i.item_code == this.last_item_code)?.image || "",
      timeleft: 5000,
    };
    this.$component.is(":visible") && this.events.load_data_display(data);
  }
  ...
  render_grand_total(value) {
    const currency = this.events.get_frm().doc.currency;
    const second_currency = this.events.get_frm().doc.custom_second_currency;
    let second_total = "";
  ...
  render_taxes(taxes) {
    if (taxes && taxes.length) {
      ...
      const taxes_html = taxes
        .map((t) => {
          if (t.included_in_print_rate) return;
  ...
  toggle_checkout_btn(show_checkout) {
    const doc = this.events.get_frm().doc;
    if (show_checkout) {
      this.$totals_section.find(".actions").css("display", "grid");
      this.$totals_section
        .find(".edit-cart-btn")
        .css({ display: "none", color: "var(--text-muted)" });
    } else {
      this.$totals_section.find(".actions").css("display", "none");
      this.$totals_section.find(".edit-cart-btn").css({
        display: "flex",
        color: !doc.custom_is_invoice
          ? "var(--text-muted)"
  ...
  highlight_checkout_btn(toggle) {
    ...
    if (toggle) {
      ...
      !doc.custom_is_invoice &&
        this.$cart_container.find(".save-btn").css({
      ...
      this.$cart_container.find(".save-btn").css({
        color: "var(--btn-default-hover-bg)",
      });
    }
  }
  ...
  on_numpad_event($btn) {
    ...
    const is_invoice = this.events.get_frm().doc.custom_is_invoice;
    ...
    if (action_is_field_edit) {
      if (!action_is_allowed) {
        ...
        const message = __("Editing {0} is not allowed", [label]);
        frappe.show_alert({
          indicator: "red",
          message: message,
        });
  ...
  render_customer_fields() {
    ...
    function handle_customer_field_change() {
      ...
      if (
        ...
        frappe.call({
          method: "ditech_core.ditech_core.pos.set_customer_info",
          args: {
  ...
  load_table() {
    ...
    if (doc?.custom_pos_table) {
      let inv_name = doc.name;
      frappe.db
        .get_value("POS Table", doc?.custom_pos_table, [
          "label",
          "invoice_split",
          "pos_invoice",
          "name",
        ...
        .then((r) => {
          let res = r.message;
          if (res) {
            let table_html = `( <span id="table-label" title="${
              res.label
            }">${frappe.ellipsis(res.label, 15)}</span> )`;
            me.$table_label.html(table_html);

            me.$component.find(".invoice-section").html("");
            if (res.invoice_split || res.pos_invoice) {
              let filters = res.invoice_split
                ? res.invoice_split.split(" , ")
                : [res.pos_invoice];

              me.invoice_field = frappe.ui.form.make_control({
                df: {
                  ...
                  onchange: function () {
                    if (this.value && inv_name != this.value) {
                      inv_name = this.value;
                      me.events.load_doc(this.value);
                    }
                  },
                  get_query: function () {
                ...
                parent: me.$component.find(".invoice-section"),
                render_input: true,
              });
              me.invoice_field.toggle_label(false);
              me.invoice_field.set_value(inv_name);
            }
            doc.custom_pos_table = res.name;
          }
        });
    }
  }
  ...
  load_invoice() {
    ...
    this.fetch_customer_details(frm.doc.customer).then(() => {
      if (!frm.doc.custom_guest_number && !this.guest_count)
        frm.set_value("custom_guest_number", 1);
  ...
  toggle_component(show) {
    !show && this.events.load_clear_display();
